...
//Nota: mi_mapa es un apuntador, para usarlo se
//sugiere desrreferenciarlo con el operador *
...
//(devuelve un set que contenga unicamente los valores que
//mi_set1 y mi_set2 tengan en comun)
set<int> getInserseccion(set<int> mi_set1, set<int> mi_set2)
{
    set<int> mi_set3;
    for(set<int>::iterator i = mi_set1.begin();
        i!=mi_set1.end();
        i++)
    {
        if(mi_set2.find(*i)!=mi_set2.end())
            mi_set3.insert(*i);
    }
    return mi_set3;
}
...
//(devuelve un set que contenga todos los valores de
//mi_set1 y mi_set2)
set<int> getUnion(set<int> mi_set1, set<int> mi_set2)
{
    ...
    for(set<int>::iterator j = mi_set2.begin();
        j!=mi_set2.end();
        j++)
    {
        mi_set3.insert(*j);
...
//devuelve true si mi_sub_set (dado) es un subconjunto
//de mi_set (dado)
//(mi_set contiene todos los valores de mi_sub_set)
bool esSubConjunto(set<int> mi_set, set<int> mi_sub_set)
{
    for(set<int>::iterator i = mi_sub_set.begin();
        i!=mi_sub_set.end();
        i++)
    {
        if(mi_set.find(*i)==mi_set.end())
            return false;
    }
